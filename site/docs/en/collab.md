...
```mermaid
flowchart TB
    A["💬 Describe the need in plain language"] --> B["🤖 AI analyzes · options table (A/B/C)"]
    B --> C["🧭 Human picks direction / redirects"]
    C --> D["⚙️ AI implements + runs automated tests"]
    D --> E["👀 Human reviews & verifies"]
    E --> F["🚀 Ship · verify after deploy"]
    F -. next request .-> A

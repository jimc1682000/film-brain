...
- All 667 titles in the library are films — **nothing related to MJ at all** (no concert films, no music documentaries)
...
→ Threshold set at **0.45** (the middle of the gap), hot-reloadable config.

## 4. The fix

1. **Confidence gate**: best original-query cosine < 0.45 → the library has no
   true match; the page shows an honest warning — while still offering
   best-guess results as starting points
2. **Display bands**: relative scores map into a band — [55%, 95%] with real
   matches, [30%, 65%] in low confidence. **No more fake 100%**; even tag
   boosts can't push a guess back to green
3. **Re-min-max within the visible slice**: the shown top-k stretches across
   the whole band, turning the huddled 65/65/62/60/59… into a clear 65 → 30
   gradient (ordering untouched)
4. **Explainable cards**: every result states which conditions it matched and
   how it was found (semantic / imagined / lexical)

## 5. Outcome

The same query after the fix: the warning banner appears, scores cap at 65% —
and because the "music / dance" condition boosts now work properly, the films
that surface are actually music-related. **Better than before the fix.**
Honesty didn't cost the experience; it made the guesses usable.
